...
void inputValueOnKeyDown(int keycode) {
    if (!onEnterNameScene) return;

    if (keycode == ALLEGRO_KEY_BACKSPACE && nameInputLen > 0) {
        nameInput[--nameInputLen] = '\0';
    }
    else if (keycode >= 1 && keycode <= 26 && nameInputLen < 17) {
        nameInput[nameInputLen++] = keycode - 1 + 'A';
    }
}
...
void start_loop(void) {
    ...
    while (!gameDone) { // Will loop until the program exitd
        ...
        case ALLEGRO_EVENT_KEY_DOWN:
            condition = true;
            inputValueOnKeyDown(event.keyboard.keycode);
